...
// ─── SHIPMENT LIST ────────────────────────────────────────────────────────────
function ShipmentListScreen({ user, shipments, loading, customTemplates, onSelect, onNew, onNewTemplate, onLogout }) {
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)", zIndex: 200, display: "flex", alignItems: "flex-end", justifyContent: "center" }}
      ...
      <div style={{ background: C.white, borderRadius: "20px 20px 0 0", padding: "20px 20px 44px", width: "100%", maxWidth: 500, maxHeight: "90vh", overflowY: "auto" }}>
        ...
        <input autoFocus placeholder="e.g. Vitabiotics May 2026 or PO-1045"
          ...
          onKeyDown={e => e.key === "Enter" && ready && onCreate(name.trim(), templateId)}
        ...
        <button onClick={() => ready && onCreate(name.trim(), templateId)} disabled={!ready || creating}
...
export default function App() {
  ...
  const handleCreate = async (name, templateId) => {
    setCreating(true);
    const { data, error } = await supabase.from("shipments").insert({ name, template_id: templateId, completed: {}, created_by: user.name, last_updated_by: user.name }).select().single();
    setCreating(false);
    if (!error && data) {
